                    ...
                    <TableCell truncate={false} className="w-[min-content] align-top">
                      <div className="space-y-1">
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="mt-2">
                              <p className="font-medium text-muted-foreground text-sm print:text-xs">
                                {_(msg`Fields Not Shown (Conditional)`)}:
                              </p>
                              <ul className="mt-1 space-y-0.5">
                                {skipped.map((log) => (
                                  <li key={log.data.fieldId} className="text-muted-foreground text-sm print:text-xs">
                                    <span className="font-medium">{log.data.fieldLabel || log.data.fieldId}</span>
                                    {log.data.unmetRuleSummary ? (
                                      <>
                                        {' '}
                                        &mdash; {_(msg`not shown because`)} {log.data.unmetRuleSummary}
                                      </>
                                    ) : null}
                                  </li>
                                ))}
