...
import { resolveFieldOverflowMode, FIELD_DEFAULT_GENERIC_ALIGN } from '../../types/field-meta';
...
const createFieldSignature = (field: FieldToRender, options: RenderFieldElementOptions): FieldSignature => {
  const { pageWidth, pageHeight, mode = 'edit', translations, signatureFontFamily, signatureFontSize } =
    options;
  ...
  // Precedence: per-field override wins, then the org/team default, then the
  // font upstream picks from the text itself.
  const resolvedFontFamily = signatureFontFamily
