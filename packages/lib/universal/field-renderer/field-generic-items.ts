...
export const upsertFieldRect = (field: FieldToRender, options: RenderFieldElementOptions): Konva.Rect => {
  ...
  fieldRect.setAttrs({
    ...
    height: fieldHeight,
    fill:
      fieldCanvasStyle?.backgroundColor ??
      (color ? getRecipientColorStyles(color).fieldBackground : DEFAULT_RECT_BACKGROUND),
    stroke:
      fieldCanvasStyle?.borderColor ??
      (color ? getRecipientColorStyles(color).fieldBorder : '#e5e7eb'),
